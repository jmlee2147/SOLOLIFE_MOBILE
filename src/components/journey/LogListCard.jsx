...
export default function LogListCard({
  ...
  reactionsCount = 0,
  onPress,
  onPressOptions,          // ← 아이콘 앵커 좌표를 부모로 전달
  onToggleLike,
  onToggleBookmark,
  style,
  placeholderImage,
  ...
  const handlePressOptions = useCallback(() => {
    ...
    const node = moreAnchorRef.current;
    ...
    <Pressable onPress={onPress} style={[styles.card, style]} accessibilityRole="button">
      <View style={styles.thumbWrap}>
        {thumbnail ? (
          ...
          >
            {placeholderImage && (
              <Image source={placeholderImage} style={{ width: 70, height: 70 }} resizeMode="contain" />
            )}
          </View>
        )}
      </View>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.titleRow}>
          <Text className="flex-1 text-heading-2 font-pretendardSemiBold" numberOfLines={1}>
            {title}
          </Text>

          {isMine ? (
            // 내 기록 → 옵션 아이콘 (측정용 래퍼 필요, collapsable=false)
            <View ref={moreAnchorRef} collapsable={false}>
              <Pressable onPress={handlePressOptions} hitSlop={8}>
        ...
        <View style={styles.placeRow}>
          ...
          <Text className="ml-1 text-body-2 text-gray700 font-pretendardMedium" numberOfLines={1}>
        ...
        <View style={styles.footer}>
          {isMine ? (
            <Text className="text-body-3 font-pretendardRegular text-gray700" numberOfLines={1}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <View style={styles.actionItem}>
              ...
              <Text className="ml-1 text-body-2 text-gray500 font-pretendardMedium">{commentsCount}</Text>
            </View>
            <View style={styles.actionItem}>
              <Icon name="smile" width={16} height={16} />
              <Text className="ml-1 text-body-2 text-gray500 font-pretendardMedium">{reactionsCount}</Text>
            </View>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  card: { flexDirection: "row", overflow: "hidden", minHeight: 108 },
  thumbWrap: { width: TH, marginRight: 11, position: "relative", alignSelf: "stretch" },
