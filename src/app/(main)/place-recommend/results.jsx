...
// 실패/오프라인 시 목업
const MOCK_ITEMS = [
  { location_id: 1, location_name: "55데시벨", category: "카페", address: "경기도 수원시 영통구", latitude: 37.248492, longitude: 127.076754, rating_avg: 4.5, photos: [], keywords: ["조용한", "디저트"], features_flat: ["아늑함"] },
  { location_id: 2, location_name: "앤드카페", category: "카페", address: "경기도 수원시 영통구", latitude: 37.2512, longitude: 127.0719, rating_avg: 4.2, photos: [], keywords: ["감성적인"], features_flat: [] },
  { location_id: 3, location_name: "북서울꿈의숲", category: "공원", address: "서울 강북구", latitude: 37.6512, longitude: 127.0386, rating_avg: 4.7, photos: [], keywords: [], features_flat: [] },
];
...
export default function ResultsScreen() {
  ...
  const categoryLabel = useMemo(() => keyToLabel(String(category || ""), String(subcategory || "")), [category, subcategory]);
  ...
  const centerForAPI = useMemo(
    () => (centerFromJson?.lat && centerFromJson?.lng && centerFromJson) || centerFromLatLng || { lat: 37.2421, lng: 127.0719 },
  ...
  const fetchRecs = useCallback(async () => {
    setLoading(true); setUsedMock(false);
    try {
  ...
  const contentPadding = useMemo(() => ({ paddingHorizontal: SPACER, paddingVertical: vs(8) }), [SPACER]);

  const listRef = useRef(null);
  const [liked, setLiked] = useState({});
  const scrollX = useRef(new Animated.Value(0)).current;
  const dotIndex = useMemo(() => Animated.divide(scrollX, CARD_W + ITEM_GAP), [scrollX, CARD_W, ITEM_GAP]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const rafRef = useRef(null);
  const snapSize = CARD_W + ITEM_GAP;

  // 스크롤 중에 바로바로 index 갱신(가볍게 rAF로 디바운스)
  const onScrollFast = useCallback((e) => {
      const x = e?.nativeEvent?.contentOffset?.x ?? 0;
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      rafRef.current = requestAnimationFrame(() => {
        const idx = Math.round(x / (CARD_W + ITEM_GAP));
        setCurrentIndex((prev) => (prev === idx ? prev : Math.max(0, Math.min(items.length - 1, idx))));
      });
    }, [items.length, CARD_W, ITEM_GAP]);

  const scrollToIndex = useCallback((index) => {
    if (!listRef.current) return;
    listRef.current.scrollToOffset({ offset: index * snapSize, animated: true });
  }, [snapSize]);
  ...
  const renderItem = useCallback(
    ({ item, index }) => {
      ...
      const scale = scrollX.interpolate({ inputRange, outputRange: [0.86, 1.0, 0.86], extrapolate: "clamp" });
      const opacity = scrollX.interpolate({ inputRange, outputRange: [0.7, 1.0, 0.7], extrapolate: "clamp" });
      ...
      return (
        <Animated.View style={{ width: CARD_W, marginRight: ITEM_GAP, transform: [{ scale }], opacity }}>
          <PlaceCard
            ...
            onToggleLike={() => setLiked((p) => ({ ...p, [item.location_id]: !p[item.location_id] }))}
            onPressTitle={() => {
    ...
    [CARD_W, ITEM_GAP, liked, scrollX, currentIndex, scrollToIndex, selectedMoods, selectedKeywords]
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View style={{ paddingHorizontal: 25, paddingTop: 5, backgroundColor: "#FFFFFF" }}>
        ...
        <View className="flex-row flex-wrap mt-[3px] mb-[31px]">
          {[...selectedMoods, ...selectedKeywords].slice(0, 3).map((k) => (
            <View key={k} className="px-[11px] py-[3px] mr-2 rounded-full border border-gray200">
      ...
      {/* 페이지네이션 점 (FlatList 위 / 버튼 위) */}
      {!loading && items.length > 0 && (
        <View style={{ position: "absolute", left: 0, right: 0, bottom: 85, alignItems: "center" }}>
